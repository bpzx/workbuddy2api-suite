...
### 重要：wb2api 上游已删除，该部分改由本项目维护

`Sliverkiss/workbuddy2api` 已不可访问（仓库 API 与网页均 404；账号仍在，其公开
仓库列表中已无该项目）。**manager 上游正常。**

影响与应对：

- **构建不受影响** —— `vendor/wb2api` 是自包含快照，构建期不访问上游。这正是当初
  选择 vendoring 而不是 submodule 换来的保险。
- **不再有可同步的目标**：`--only wb2api` 会 fetch 失败；每日漂移检测对它只记一行
  "查询失败"（fail-soft，不开 issue 也不失败）。
- 本地镜像缓存 `.upstream-cache/wb2api.git` 保留到 `f2ccc7b`（上游删除前最后可见
  HEAD）的完整历史，可备查阅。**最后一版上游产品代码**是我们锁定的 `d1023f3`；
  之后上游只推进过自己的 CI 治理机器人。
- **界线变了**：以前"`vendor/` 一字不改 + 构建期补丁"是为了保持可同步，现在补丁
  实际是**永久本地改动**。当前仍走补丁机制（改动集中、一处可审计），若本地改动
  继续增多可考虑把 wb2api 正式"收养"为一等源码目录。
- 文档相应更新：`UPSTREAMS.md` 新增「wb2api 上游已删除」一节并把锁定表里那一行的
  死链去掉；`MAINTAINING.md` 说明该部分自行维护。

### 风控加固（针对账号安全，多为上游从未有过的能力）

借此做了一次完整的风控点审计，处理如下。**每一条都做了针对性验证**（见文末）。

- **P0 设备风控凭据可见化**：新增 `docker/overlay/check_device_token.py`，启动时
  只读扫描 auth 目录，若账号缺 `device_token` 就打印「N/总数 个账号缺少
  device_token」。为什么必须提示：网关对外**声称自己是官方桌面客户端**，而
  `X-Device-Token` 只在账号带该字段时才发送，**内置登录流程不写它** —— 缺了不报错、
  只是**静默降级风控形态**（上游 manager 侧注释原话；它甚至在换 token 重登时专门
  保留该字段）。工具**只报数量、不打印凭据**，且永远返回 0，不影响启动。
- **P3 设备标识可轮换**（补丁 5）：`X-Machine-ID`/`X-Session-ID` 由
  `sha256("<盐>" + purpose + ":" + uid)` 稳定派生，盐是硬编码常量；一旦某账号设备
  标识被上游打标，**重新登录也换不掉**。现支持 `WB2A_DEVICE_ID_SALT` 覆盖，
  **默认仍取历史常量（行为逐字节不变）**。这是本套件设置的唯一 `WB2A_*` 变量
  （它不在上游 `applyEnv` 名单里，与设置页无冲突）。
- **P4 `max_in_flight: 0` 告警**（补丁 6）：0 = 单账号在途**不限**，该值只能在
  `config.json` 里写、误设无任何报错，而并发无上限会显著提高账号风控风险。
  现设成 0 时启动日志会 WARN。
- **P5 上报间隔抖动**（补丁 7）：签到/旅行/活跃上报的间隔是硬编码的 0.8s / 1.5s，
  精确等距是"机器特征"。现加 0..+50% 抖动（新增 `jitteredDelay`）。`base<=0` 原样
  返回，测试靠置 0 跳过等待，不受影响。
- **客户端版本号显式化**（P2）：`upstream.client_version` / `cli_version` 在模板里
  从空串改为显式契约版本（`5.5.4` / `2.137.1`），使设置页「出站标识」能直接看到
  实际使用的版本，便于随官方客户端升级而更新。

**未处理、但已如实记录为未决项的**（理由见 `MAINTAINING.md`）：合成任务**载荷**
在所有账号间同构（改动它等于编造事件数据，收益/风险需判断）；`cmd/login` 与网关的
UA 版本不一致（盲改可能弄坏加号流程，需单独验证）；`max_in_flight_global` 注释与
实现不符（行为是有意的，改注释属改上游文案，已在 README 说明以 `2` 为准）。

**验证方式**（这三项触及对上游的网络行为，不能只靠"测试全绿"）：

| 项 | 验证 | 结果 |
|---|---|---|
| 补丁 5 | 与 `sha256("wb2a:machine:u1")[:36]` 手算值逐字节比对 + 空白回落 + 换盐生效 + 幂等 | 通过 |
| 补丁 6 | **真启动服务**：`max_in_flight=0` → WARN；`=3` → 无 WARN | 通过 |
| 补丁 7 | 300 次采样统计不同值个数 | 修复前 **1** 个 → 修复后 **300** 个 |
| 回归 | Go `build` / `vet` / `test ./...` | 18/18 包通过 |

> 补丁 7 的**初版正是被这个采样测试否掉的**：初版用 `time.Now().UnixNano()` 取熵，
> 在时钟粒度粗的平台（实测 Windows）上同一值重复 300 次，抖动退化成常数 ——
> 已改用 `math/rand`。这条记在 `UPSTREAMS.md` 的补丁登记里。

### 变更：README 精简（含维护者手改，此处按**最终状态**记录）

README 大幅裁减，去掉了多节"要不要选这个项目"式的推销内容与警诫性章节，
并新增了一节机器配置建议。

**删除的章节**：

- **「与直接用上游的区别」**（对照表 + 「设计取舍」说明）：对照表是面向选型的
  推销性内容；取舍理由（为什么不为上游文案打补丁、面板为何是整文件覆写）在
  [`UPSTREAMS.md`](UPSTREAMS.md) 的补丁登记里有权威记录，删掉不丢信息。
- **「不建议这样部署」** 与 **「平台说明」**。其中两条建议**没有丢**：
  `MANAGER_BIND` 与「用 Cloudflare Tunnel 上线」写在 [`.env.example`](.env.example)
  里（那才是配置它时会看的地方）；NFS 的属主问题同样在 `.env.example` 的说明里。
  「平台说明」的 **Windows 注意事项**折成「已知限制」表格里的一行
  （去掉整节，但保留了这个此前遗漏、且真实有效的注意事项）。
- **维护者随后手工删掉**了「部署机要求」的硬性要求表（Docker + Compose v2、
  仅 amd64/arm64、出网、数据目录需本地且可 chown、宿主时钟）。
  ⚠️ 如实指出代价：**「Compose v2 必需」与「仅构建 amd64/arm64」这两条约束
  现在没有写在任何地方**（其余几条在 `.env.example` 的对应配置项旁仍有体现）。
- **保留**：上游快照与来源可追溯的陈述、**合规须知**（法律声明）—— 这两样
  原本夹在被删的章节里，现位于文档开头。后者是法律声明，不该随精简一起消失。

**保留并调整的**：

- 「建议配置」（CPU 1–2 核 / 内存 512 MB–2 GB / 磁盘 2–5 GB，写明各自受什么
  影响）保留，提升为独立的 `##` 节。磁盘数字明确标注为**预期值**
  （本项目未实跑过镜像构建，不编精确数字），并给出自查命令
  （`docker image inspect` 量镜像、`docker stats` 量运行占用）。

**连带修正**：

- `UPSTREAMS.md` 里指向已删章节的悬空引用（否则读者会找不到目标）。
- `docker-compose.yml` 头部注释里过时的"三个容器"（加入 updater 后是四个）。
