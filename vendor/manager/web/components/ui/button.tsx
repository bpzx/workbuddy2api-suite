...
const buttonVariants = cva(
    /*
      按钮内图标的尺寸与清晰度在 globals.css 里统一定义（那里能利用 CSS 层级：
      显式尺寸工具类 h-3.5 / size-5 会自动覆盖默认值，不必写脆弱的
      `[&_svg:not([class*=...])]` 守卫——之前的写法编出来的选择器根本匹配不上，
      见 globals.css 中 `[data-slot="button"] svg.lucide` 的注释）。
    */
