...
### 新增：README 的「部署机要求与建议配置」

README 开头新增一节，把"要一台什么样的机器"讲清楚 —— 此前只有 `快速开始`
下面两行 `前置条件`（Docker + 账号），不足以判断能不能部署、要留多少资源。

- **必须满足**：Docker Engine + **Compose v2**（compose 用了顶层 `name:`）、
  `linux/amd64` 或 `linux/arm64`（CI 只构建这两个）、能出网（ghcr / GitHub API /
  腾讯，或不畅时配 `WB_HTTP_PROXY`）、数据目录在**本地文件系统**且可 `chown`
  （NFS 会失败）、宿主时钟准确（镜像内固定 `TZ=Asia/Shanghai`）。
- **建议配置**：CPU 1–2 核 / 内存 512 MB–2 GB / 磁盘 2–5 GB，并写明各自受什么
  影响。**磁盘数字明确标注为"预期值、未在仓库内实测"**，并给出自查命令
  （`docker image inspect` 量镜像、`docker stats` 量运行占用）—— 本项目没有
  实跑过镜像构建，不编精确数字。
- **两件不要做**：不要把 7864 直接暴露公网（应经 CF Tunnel 并把 `MANAGER_BIND`
  设为 `127.0.0.1`，否则 IP 白名单与登录锁定失效）；不要把数据目录放 NFS。
- **平台说明**：Linux 是目标环境；Docker Desktop 可跑，但**一键更新在 Windows
  上未实测**（helper 依赖"套件目录按同一绝对路径挂载"，涉及 WSL 路径转换）——
  这一点此前只在我的对话里提过，没写进文档，属于遗漏。

顺带修掉 `docker-compose.yml` 头部注释里过时的"三个容器"（加入 updater 后是四个）。
