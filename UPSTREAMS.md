...
**本文件负责"血统与登记"**：锁定了哪些 commit、打了哪些补丁 / 覆写了哪些文件、
有哪些坑。**同步与发版的流程命令**在 [`MAINTAINING.md`](MAINTAINING.md)；
**哪个版本捆绑了哪些上游 commit** 在 [`CHANGELOG.md`](CHANGELOG.md)。
三者分工固定，内容不重复 —— 同一件事写两处必然漂移。
