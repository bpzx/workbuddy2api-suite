...
> 面板上**没有**「上游网关 workbuddy2api」这一行：那个仓库已被删除，而 wb2api
> 现在由本项目自行维护（见 [`UPSTREAMS.md`](UPSTREAMS.md)），没有"上游"可对比。
> 曾经它显示"固定提交 → 远端 HEAD"，仓库删除后查询必然失败、缓存又会把旧值留住，
> 于是**永久**亮着「有新版本可更新」—— 仓库都不存在了，谈不上更新，所以删掉了。
