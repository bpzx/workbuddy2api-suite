...
### 修复：面板「上游网关 workbuddy2api」那一行误导

wb2api 仓库删除后，面板仍显示「固定提交 `d1023f3` → 远端最新 `f2ccc7b`」并
**永久**亮着「有新版本可更新」。原因是组合出来的，两处都错：

- 查询一个已删除的仓库必然 404；
- 而"保留上次成功结果"的缓存逻辑（本意是给**临时**网络故障兜底）把旧值一直留着
  —— 于是界面拿一个不存在的远端 HEAD 去比，永远"有更新"。仓库都没了，谈不上更新。

**删掉了这一行**（面板、接口、i18n 文案一并移除），连带清掉随之失去消费方的死代码：
`same_commit()` 前缀比较、`SHORT_LEN`、`pinned_upstreams()` 里除 `repo` 外的字段。
wb2api 现在由本项目自行维护，这个只有"有无更新"语义的区块里没有它的位置。

### 修复：把「查询失败」谎报成「仓库里没有正式版 tag」

顺手修掉一个真实出现过的假警报：面板同时显示了一个具体版本号**和**
「仓库里没有正式版 tag」。原因是 `_fetch_release_tag()` 把所有异常吞掉、
统一给出那句结论，而真实原因可能是「仓库不可访问（404）」或
「未认证接口限流（403）」——与"确实没有 tag"完全是两回事。

现在：

- 失败原因**如实区分并透出**（404 → 不可访问；403 → 可能已限流；其它 → 带 HTTP 码）；
- 仍然保留上次成功的结果（临时故障不该让界面从"有更新"掉成"未知"），
  但错误文案会**说明**这一点：「…；显示的版本号来自上次成功检测」，
  不再让"有值"和"报错"并排却不说清关系。

新增 `GhReasonTest` / `FetchReleaseTagTest` / `FetchAllRepoTest` 覆盖这三条，
其中 `FetchAllRepoTest` 钉住「不再查已删除的 wb2api 仓库」（断言实际请求的仓库
列表只有套件与 manager）。

### 同步上游：manager v1.0.60 → v1.0.70（跨 10 个发版）

`3fb56bd` → `1a48761`（142 文件 / +18092 −455，至本次同步为止最大的一次）。
wb2api 侧无可同步目标（上游仓库已删除，该部分由本项目自行维护）。

用户可见的主要变化（全部来自上游）：

- **红包**：一次生成一批带额度的密钥 + 公开抽奖页（每 IP 一次）；密钥列表按
  「普通 / 红包」分组；收回红包改为**删除**密钥。
- **作用域化 API Token**（管理面 P0+P1）：细粒度权限 + 权限矩阵守卫。
- 请求日志显示**实际调用的账号**与**提示词缓存命中**情况（issue #69）。
- `thinking.type=adaptive` 被接受，修复思考过程不可见（含行为级用例）。
- 均分红包可能生成无限额度密钥的修复（额度恒为正）。
- 仪表盘首屏如实呈现状态（骨架 / 错误态 / 刷新失败提示）。
- 模型中心按厂商官方资料区分原生文本与多模态能力。
- 账号备注、密钥列错位修复、「全部签到」不算国际版账号等。
- 上游在 v1.0.67 记录了「**上游（wb2api）已停更**」并支持 `UPSTREAM_SRC` 本地
  源码安装 —— 与本项目"wb2api 自行维护"的判断相互印证。

**集成层核对**：6 个补丁目标文件中 5 个未被动；唯一动到的 `main.py` 是导入块
（上游新增 redpackets / tokens 两个路由模块），已按新收尾行更新锚点 —— 预检测试
在同步后立刻报出，随后全绿。被覆写的 `UpdatePanel.tsx` 未被动，挂载点与
`http`/`errText` 导出完好；语系仍 5 个，`suiteUpdate` 合并校验通过。

> `deploy/update.py` **这次确实改动了**（+314/−39，上游为应对"自己的上游停更"
> 加了 `UPSTREAM_SRC` 本地源码安装等路径）。但它对我们的 stub **无影响**：
> stub 覆盖的就是这个文件本身，而它与管理端之间的契约由**读取端**
> `server/services/updater.py` 定义 —— 后者**未变**，所以状态 JSON 的 schema
> 依然对得上。（同步脚本的检查清单里"上游改了 `/api/system` 约定要复核 stub"
> 这条，判据应是**读取端**是否变化，而不是被覆盖的文件是否变化。这里记一笔，
> 免得下次照着改错东西。）

**验证**：上游 Python 全套 1571 项（4 个错误已核实为 Windows 专有的迁移测试
目录问题，未打补丁的原始快照同样失败，与本次改动无关）；Go 18/18 包；前端静态
导出编译通过，产物含我们的端点与文案、不含已移除的行话；本项目 113 项 + 预检
13 项全绿；配置模板无漂移。
